...
# Install base packages (including gosu for dropping privileges)
...
# Create rails user for running the app (container starts as root for volume permission fixes)
RUN groupadd --system --gid 1000 rails && \
    useradd rails --uid 1000 --gid 1000 --create-home --shell /bin/bash && \
    mkdir -p /rails/storage/files && \
    chown -R rails:rails /rails/storage
# Note: Not using USER here - entrypoint will fix volume permissions then drop to rails user
